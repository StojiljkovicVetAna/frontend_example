...
def create_morph_animation(img1, img2, landmarks1, landmarks2, num_frames=30):
    """Create animated morphing sequence"""
    frames = []
    for i in range(num_frames):
        alpha = i / (num_frames - 1)
        morphed = morph_faces(img1, img2, landmarks1, landmarks2, alpha)
        morphed_rgb = cv2.cvtColor(morphed, cv2.COLOR_BGR2RGB)
        frames.append(Image.fromarray(morphed_rgb))
    return frames
    ...
    if older_file and younger_file:
        ...
        try:
            ...
            with col1:
                ...
                if st.button("Generate Single Morph", type="primary"):
            ...
            with col2:
                num_frames = st.slider("Animation frames", 10, 60, 30, 5)
                fps = st.slider("Frames per second", 5, 30, 15, 5)
                
                if st.button("Generate Animation", type="secondary"):
                    with st.spinner(f"Generating {num_frames} frames..."):
                        frames = create_morph_animation(older_cv, younger_cv, landmarks_older, landmarks_younger, num_frames)
                        
                        st.subheader("Animated Morph")
                        ...
                        st.download_button(
                            ...
                            file_name="age_morph_animation.gif",
                            mime="image/gif"
                        )
